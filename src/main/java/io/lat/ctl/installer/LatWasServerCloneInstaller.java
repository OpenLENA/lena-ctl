...
/**
 * lat-was 를 복제하기 공통 로직을 담고 있는 Installer class
 */
public class LatWasServerCloneInstaller extends LatInstaller{
	/**
	 * 기본생성자
	 * @param installCommandType
	 * @param installServerType
	 */
	public LatWasServerCloneInstaller(InstallerCommandType installCommandType, InstallerServerType installServerType) {
	...
	public void execute() {
		...
		if(!InstallInfoUtil.existsServer(srcServerId)){
			throw new LatException(srcServerId + " doesn't exist.");
		}
		...
		if(!FileUtil.exists(srcInstallPath)){
			throw new LatException(srcInstallPath + " doesn't exist.");
		}
		...
		if(InstallInfoUtil.existsServer(cloneServerId)){
			throw new LatException(cloneServerId + " already exists.");
		}
	...
	protected HashMap<String, String> getServerInfoFromUser() {
		...
		System.out.println("|    ex : lat_was-8090                                                               ");
		System.out.print("|: ");
